...
struct VideoChecker: View {
    ...
    private func validateDetectedObjects() {
        ...
        let persons = detectedObjects.filter { $0.name == "person" && $0.confidence * 100 >= 70 }
        if persons.count == 1 {
            isVideoValid = true
            validationHandler(true)
            message = "Validation successful. Navigate back and proceed to video analysis."
        } else {
            isVideoValid = false
            validationHandler(false)
            message = "Validation failed. Please ensure exactly one person is present in the video with clear visibility and read the instruction how to take a video."
        }
    }
