...
//            }
            ...
            var annotatedFrames = frames.compactMap { frame -> UIImage? in
            ...
            DispatchQueue.main.async {
                ...
                self.totalFramesProcessed = frames.count // Store the total frames processed
                self.isProcessing = false
            }
        }
    }
...
struct ExpandableAngleBox: View {
    ...
    var body: some View {
        let info = AngleInfoProvider.getInfo(for: label, minAngle: minAngle, maxAngle: maxAngle, recommendedRange: recommendedRange)
        ...
        DisclosureGroup(
            ...
            content: {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("Minimal analysed angle: \(Int(minAngle))°")
                        .font(.caption)
                    Text("Maximal analysed angle: \(Int(maxAngle))°")
                        .font(.caption)
                    // Recommended range
                    Text("Analysed angles on recommended range scale:")
                        .foregroundColor(.gray)
                        .font(.footnote)
                    ...
                    if !info.recommendations.isEmpty {
                        ...
                        ForEach(info.recommendations, id: \.self) { recommendation in
                            Text("• \(recommendation)")
                                .font(.footnote)
            ...
            label: {
                HStack {
                    ...
                    Spacer()
                    Text("\(Int(minAngle))° to \(Int(maxAngle))°")
                        .font(.headline)
                        .foregroundColor(.white)
