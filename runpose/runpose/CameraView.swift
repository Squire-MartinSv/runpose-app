        ...
        NavigationStack {
            VStack {
                ...
                if let videoURL = selectedVideoURL, isVideoUploadedOK {
                    ...
                    Text(isVideoValidated ? "Video is ready for analysis." : "Video is ready to be validated.")
                        .foregroundColor(isVideoValidated ? .green : .gray)
                        .padding(.bottom, 5)
            ...
            .navigationDestination(isPresented: $navigateToPoseAnalyzer) {
                if let videoURL = selectedVideoURL {
                    // Debugging print statement moved outside of the View builder
                    //Text("Navigating to VideoAnalyser with URL: \(videoURL)")
                    VideoAnalyser(videoURL: videoURL)
                } else {
                    // Debugging print statement moved outside of the View builder
                    //Text("Navigate to Pose Analyzer triggered with no video URL")
                    Text("No video selected.")
                        .foregroundColor(.red)
                }
            }
    ...
    class Coordinator: NSObject, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
        ...
        func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey: Any]) {
            if let videoURL = info[.mediaURL] as? URL {
                //print("Selected video URL: \(videoURL)") // Debugging
                ...
                Task {
                    do {
                        ...
                        DispatchQueue.main.async {
                            let roundedDuration = String(format: "%.2f", durationSeconds) // Round to 2 decimal places
                            if durationSeconds >= 2.0 && durationSeconds <= 5.0 {
                                self.parent.selectedVideoURL = videoURL
                                self.parent.isVideoUploadedOK = true
                                self.parent.alertMessage = "Video has been uploaded successfully. Duration: \(roundedDuration) seconds."
                            } else {
                                self.parent.isVideoUploadedOK = false
                                if durationSeconds < 2.0 {
                                    self.parent.alertMessage = "The selected video is shorter than 2 seconds. Please select a video between 2 and 5 seconds to optimize analysis process."
                                } else {
                                    self.parent.alertMessage = "The selected video is longer than 5 seconds. Please select a video between 2 and 5 seconds to optimize analysis process."
                                }
                            }
